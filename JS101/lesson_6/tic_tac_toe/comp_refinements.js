...
function computerChoosesSquare(board) { // marks an O on the board
  ...
  for (let index = 0; index < WINNING_LINES.length; index++) { // offensive move by comp
  ...
  if (!square) { // comp picks random square
...
while (true) {
  ...
  while (true) {
    ...
    if (answer === 'y') {
      ...
      prompt('That was an invalid choice. Please respond with "y" or "n"');
    }
  }
...
// I did not include the last refinement.